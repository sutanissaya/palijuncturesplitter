...
"""
Simply type or paste in some Pāḷi text (there is a couple of resources below) you would like to juncture-split into the box then click anywhere outside the box (or press Ctrl+Enter) to have the app instantaneously render the result.\n
...
st.markdown("<p>It works by adding a <em>juncture sign</em> after every <em>heavy syllable</em> (garu akkhara). The juncture signs act as a temporal punctuation or visual spacers. The split text would then be read by dragging every syllable that is followed by a juncture sign. There are also double juncture signs next to pause-bearing punctuation marks (listed above) and at the end of lines that signify a long pause where chanters could catch their breath.</p>", unsafe_allow_html=True)
